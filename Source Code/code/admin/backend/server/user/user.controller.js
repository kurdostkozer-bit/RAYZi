...
const updateLevel = async (userId, res) => {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(userId, { $set: { level: updatedLevel } }, { new: true })
      .select("-notification -plan -roomName -roomWelcome -roomImage -linkType -imageType -link -video -post -isFake -isDemo -isBusy -bankDetails -agency")
  ...
  try {
    ...
    if (req.body.loginType == 0) {
      ...
      if (userQuery) {
        ...
        userQuery.lastLogin = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
...
exports.getProfileUser = async (req, res) => {
  try {
    ...
    const [user, profileUser] = await Promise.all([
      ...
      User.findOne({ ...query })
        .populate("level")
        .select("name username gender age image country uniqueId bio followers following video post level isVIP coverImage loginType avatarFrameImage countryFlagImage isAgency isHost isCoinSeller"),
    ]);
...
exports.search = async (req, res) => {
  try {
    const [user, blockedUserIds] = await Promise.all([
      ...
      Block.distinct("toUserId", { userId: new mongoose.Types.ObjectId(req.body.userId) }),
    ]);
...
exports.blockUnblock = async (req, res) => {
  try {
    ...
    if (user.isDemo) {
      return res.status(200).json({ status: false, message: "This is demo user , can't block!!" });
    }
...
exports.AddFakeUser = async (req, res) => {
  try {
    ...
    user.loginType = 3;
    user.analyticDate = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
    user.username = req.body.username.trim();
