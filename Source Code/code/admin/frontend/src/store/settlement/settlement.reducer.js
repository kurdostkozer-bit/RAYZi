...
export const settlementReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ActionType.ACTION_UPDATE_PENDING:
      return {
        ...state,
        agencySettlement: state.agencySettlement?.map((item) => {
          if (item?._id === action.payload.id) {
            return {
              ...item,
    ...
    case ActionType.OPEN_DIALOG_SETTLEMENT_BANK_DETAILS:
      return {
        ...state,
        dialog1: true,
        dialogData1: action.payload || null,
      };
    case ActionType.CLOSE_DIALOG_SETTLEMENT_BANK_DETAILS:
      return {
        ...state,
        dialog1: false,
        dialogData1: null,
