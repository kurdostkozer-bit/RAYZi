...
//        currency = "USD";
        ...
        if (paymentGateways.contains(STR_GP)) {
            ...
            bottomSheetPaymentBinding.lytgooglepay.setOnClickListener(v -> {
                ...
                if (dataItem.getProductKey().equalsIgnoreCase("android.test.purchased")){
                    new PopupBuilder(VipPlanActivity.this).showSimplePopup(getString(R.string.this_is_a_demo_version),  getString(R.string.ok), new PopupBuilder.OnPopupClickListner() {
                        @Override
    ...
    private void initData() {
        ...
        selectedPlan1 = sessionManager.getVipPlan().get(0);
        vipPlanAdapter.addData(sessionManager.getVipPlan());
        selectedPlan.setValue(sessionManager.getVipPlan().get(0));
...
//                binding.rvBanner.smoothScrollToPosition(pos);
    ...
    private void displayAlert(@NonNull String title, @Nullable String message, boolean b) {
        ...
        builder.setPositiveButton("Ok", (dialog, which) -> {
            if (b) {
                Toast.makeText(this, R.string.purchase_done , Toast.LENGTH_SHORT).show();
            } else {
    ...
    void onPaymentSheetResult(final PaymentSheetResult paymentSheetResult) {
        if (paymentSheetResult instanceof PaymentSheetResult.Canceled) {
            ...
            Toast.makeText(VipPlanActivity.this, R.string.payment_done , Toast.LENGTH_SHORT).show();
        }
    }
}