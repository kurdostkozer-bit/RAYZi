        ...
        call.enqueue(new Callback<RestResponse>() {
            ...
            public void onResponse(Call<RestResponse> call, Response<RestResponse> response) {
                if (response.body().isStatus() && response.isSuccessful()) {
                    Toast.makeText(VideoListGridActivity.this,  getString(R.string.deleted_successfully), Toast.LENGTH_SHORT).show();
                    customDialogClass.dismiss();
                    if (pos >= 0 && pos < profileVideoGridAdapter.getList().size()){
