...
public class UserApiCall {
                ...
                if (response.code() == 200) {
                    if (response.body().isStatus()) {
                        ...
                        Log.d("TAG", "onResponse: "+response.message());
                            onUserApiListner.onUserStatusFailed(response.body().getMessage());
                    }
                }
            }
    ...
    public void blockUnblock(String guestId , OnBlockUnblockListner onBlockUnblockListner){
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("userId",sessionManager.getUser().getId());
        jsonObject.addProperty("toUserId",guestId);

        Call<BlockUnblockRoot> call = RetrofitBuilder.create().BlockUser(sessionManager.getUser().getId(),guestId);
        call.enqueue(new Callback<BlockUnblockRoot>() {
            @Override
            public void onResponse(Call<BlockUnblockRoot> call, Response<BlockUnblockRoot> response) {
                if (response.code() == 200) {
                    if (response.body() != null) {
                        if (response.body().isStatus()) {
                            if (response.body().isIsBlocked()) {
                                onBlockUnblockListner.onBlockSuccess();
                            } else {
                                onBlockUnblockListner.onUnblockSuccess();
                            }
                        }
                    }
                }
    ...
    public interface OnBlockUnblockListner {
        void onBlockSuccess();
