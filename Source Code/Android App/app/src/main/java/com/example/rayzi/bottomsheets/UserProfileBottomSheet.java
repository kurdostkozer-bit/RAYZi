...
*/
    ...
    public void show(boolean isHost, GuestProfileRoot.User userDummy, String liveStreamingId,boolean isWatchAudioActivity) {

        if (isWatchAudioActivity || sessionManager.getUser().getId().equalsIgnoreCase(userDummy.getUserId())) {
...
//            sheetDilogBinding.btnBlock.setVisibility(View.VISIBLE);
        ...
        if (userDummy.isFollow()) {
            sheetDilogBinding.tvFollowStatus.setText(R.string.unfollow);
            sheetDilogBinding.lytFollowUnfollow.setBackgroundTintList(ContextCompat.getColorStateList(context, R.color.graylight));
        } else {
            sheetDilogBinding.tvFollowStatus.setText(R.string.follow);
            sheetDilogBinding.lytFollowUnfollow.setBackgroundTintList(ContextCompat.getColorStateList(context, R.color.pink));
        }
        sheetDilogBinding.pdFollow.setVisibility(View.GONE);
        ...
        if (userDummy.isVIP()) {
            sheetDilogBinding.layVIPType.setVisibility(View.VISIBLE);
            sheetDilogBinding.ivVIPType.setImageResource(R.drawable.crown);
        } else {
            sheetDilogBinding.layVIPType.setVisibility(View.GONE);
        }

        if (userDummy.isCoinSeller()) {
...
//            Toast.makeText(context, R.string.blocked, Toast.LENGTH_SHORT).show();
