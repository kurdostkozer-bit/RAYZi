...
public class GuestActivity extends BaseActivity {
    ...
    private void followUnfollowLogic() {
        ...
        userApiCall.followUnfollowUser(followNow, user.getUserId(), "", new UserApiCall.OnFollowUnfollowListner() {
            ...
            public void onFollowSuccess() {
                ...
                binding.tvFollowStatus.setText(getString(R.string.unfollow));
            ...
            public void onUnfollowSuccess() {
                ...
                binding.tvFollowStatus.setText(getString(R.string.follow));
                user.setFollow(true);
    ...
    private void blockUnblock(){
        ...
        binding.pdBlock.setVisibility(View.VISIBLE);

        userApiCall.blockUnblock(user.getId(), new UserApiCall.OnBlockUnblockListner() {
            @Override
            public void onBlockSuccess() {
                binding.tvBlock.setText("Unblock");
                binding.lytBlockUnblock.setBackgroundTintList(ContextCompat.getColorStateList(GuestActivity.this, R.color.graylight));
                user.setBlock(false);
                binding.pdBlock.setVisibility(View.GONE);
                binding.tvBlock.setVisibility(View.VISIBLE);
                binding.lytBlockUnblock.setEnabled(true);
                Toast.makeText(GuestActivity.this, "Blocked Successfully..", Toast.LENGTH_SHORT).show();
            ...
            public void onUnblockSuccess() {
                ...
                binding.lytBlockUnblock.setBackgroundTintList(ContextCompat.getColorStateList(GuestActivity.this, R.color.pink));
                user.setBlock(true);
                binding.pdBlock.setVisibility(View.GONE);
                binding.tvBlock.setVisibility(View.VISIBLE);
                binding.lytBlockUnblock.setEnabled(true);
                Toast.makeText(GuestActivity.this, "Unblocked Successfully..", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void initView() {
        ...
        if (user != null) {
            if (!isFinishing()) {
                if (!user.getCoverImage().isEmpty()) {
                    Glide.with(this).load(user.getCoverImage())
                            .placeholder(R.drawable.bg_cover_image)
                            .apply(MainApplication.requestOptions)
                            .diskCacheStrategy(DiskCacheStrategy.ALL)
                            .centerCrop().into(binding.imgUser1);
                } else {
                    binding.imgUser1.setImageResource(R.drawable.bg_cover_image);
                }
            }
            if (!isFinishing()) {
                binding.imgUser.setUserImage(user.getImage(), user.getAvatarFrameImage(), 30);
            }
            ...
            binding.tvUserName.setText(user.getUniqueId());
            binding.copy.setOnClickListener(v -> {
                ClipboardManager clipboard = (ClipboardManager) GuestActivity.this.getSystemService(Context.CLIPBOARD_SERVICE);
                ClipData clip = ClipData.newPlainText("", user.getUniqueId());
                clipboard.setPrimaryClip(clip);
                Toast.makeText(GuestActivity.this, "Copied Successfully..", Toast.LENGTH_SHORT).show();
            });
            ...
            if (user.isAgency()) {
                ...
                binding.ivCoinType.setImageResource(R.drawable.ic_agency);
            } else {
            ...
            if (user.isCoinSeller()) {
                binding.layCoinType.setVisibility(View.VISIBLE);
                binding.ivCoinType.setImageResource(R.drawable.ic_agency);
                binding.tvCoinType.setText("Coin Seller");
            } else {
                binding.layCoinType.setVisibility(View.GONE);
            }
